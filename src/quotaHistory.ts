...
export class QuotaHistoryTracker {
    ...
    getHourlyStats(): HourlyQuotaStats[] {
        ...
        for (let h = 0; h <= currentHour; h++) {
            ...
            if (snap) {
                // 仅当「正好一小时前」也有快照记录时才计算增量；否则视为存在时间间隙
                // （如期间 VS Code 关闭、用量发生在其他工具），不计算差值，
                // 避免把多小时的累计消耗错误地记到当前这一小时上。
                const prevHourKey = this.formatHourKey(
                    new Date(this.getDateFromHourKey(snap.hourKey).getTime() - 60 * 60 * 1000)
                );
                const prevSnap = snapMap.get(prevHourKey) || null;
                ...
                // prevSnap 不存在（前一小时无记录 / 历史首条）：deltas 保持 null
